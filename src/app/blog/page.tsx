...
export default function BlogPage() {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      setIsLoading(true);
      const response = await fetch(`/api/blog?limit=${limit}&skip=${skip}`);
      const data = await response.json();
      setPosts(data.posts);
      setTotal(parseInt(data.total) / 10 + 1);
      setIsLoading(false);
    };
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-8">
      ...
      {author && (
        <div className="my-5">
          <h5 className="text-lg font-semibold">Post by Author:{author}</h5>
        </div>
      )}
        ...
        <div className="flex-1 space-y-12">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mb-2">
                      By <Link href="#">{post.author}</Link>, {post.date}
                    </p>
                    <div className="my-2">
                      <Link href="#">
          ...
          <div className="flex justify-center items-center">
            <Pagination
              initialPage={1}
              total={total}
              onChange={() => setPage((prePage) => prePage + 1)}
            />
          </div>
        </div>
